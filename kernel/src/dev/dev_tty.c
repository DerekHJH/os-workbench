...
static int tty_init(device_t *ttydev) {
  ...
  tty->buf = dev_malloc(tty->size * sizeof(tty->buf[0]));
  tty->dirty = dev_malloc(tty->size * sizeof(tty->dirty[0]));
  tty->end = tty->buf + tty->size;
  tty->sp_buf = dev_malloc(tty->size * 2 * sizeof(struct sprite));
  for (int i = 0; i < tty->size; i++) {
  ...
  tty->cursor = tty->buf;
  struct tty_queue *q = &tty->queue;
  q->front = q->rear = q->buf = dev_malloc(TTY_COOK_BUF_SZ);
...
static ssize_t tty_write(device_t *dev, off_t offset, const void *buf, size_t count) {
  tty_t *tty = dev->ptr;
  kmt->sem_wait(&tty->lock);
...
void dev_tty_task(void *arg) {
  ...
  while (1) {
    ...
    uint32_t now = uptime(), changed = (ev.data != 0);
    if (changed || now - known_time > 500) {
      known_time = now; 

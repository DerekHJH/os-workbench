...
int fb_init(device_t *dev) {
  fb_t *fb = dev->ptr;
  fb->info = dev_malloc(sizeof(struct display_info));
  fb->textures = dev_malloc(sizeof(struct texture) * NTEXTURE);
  fb->sprites = dev_malloc(sizeof(struct sprite) * NSPRITE);
...
static ssize_t fb_read(device_t *dev, off_t offset, void *buf, size_t count) {
  fb_t *fb = dev->ptr;
...
static ssize_t fb_write(device_t *dev, off_t offset, const void *buf, size_t count) {
  fb_t *fb = dev->ptr;
  kmt->sem_wait(&fb_sem);
  if (offset == 0) {
